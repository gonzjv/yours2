...
class ModCfilteringOptions
{
     ...
     *
     * @param JDatabaseQuery a db query object
     * @param string $var_name
     * @param string $custom_field_type
     *
     * @return JDatabaseQuery a db query object
     * @since 1.5.0
     */
            ...
            if (count($ar_value) > 0) {
                if ($key != $field) {
                    ...
                    elseif ($key != 'price') {
                        ...
                        } elseif ($returned_products == 'parent') {
    ...
    public function buildCategoriesQuery(JDatabaseQuery $query, $part = false, $subtree = false)
    {
        ...
        if ($displayCounterSetting) {
            ...
            if ($this->componentparams->get('returned_products', 'parent') == 'child')
                ...
                if ($this->componentparams->get('filtered_products', 'parent') == 'parent')
                    $query->select("SUM(CASE WHEN p.product_parent_id=0 THEN 1 ELSE 0 END) AS counter");
